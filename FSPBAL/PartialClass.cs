...
namespace FSPBAL
{
    ...
    public partial class General
    {
        ...
        public static bool ValidateUploadFilePDF(byte[] DocumentByte)
        {
            System.UInt32 mimeType;
            FindMimeFromData(0, null, DocumentByte, 256, null, 0, out mimeType, 0);
            System.IntPtr mimeTypePtr = new IntPtr(mimeType);
    ...
    public partial class User
    {
        ...
        public void AuditUserOrg(string org_Code, string AssignedUserID, string CreatedUserID, string Action)
        {
            UserOrg ObjUsrOrg = db.UserOrgs.SingleOrDefault(x => x.OrgCode == org_Code && x.UserID == AssignedUserID);
            A_UserOrg ObjAuditOrg = new A_UserOrg();
            if(ObjUsrOrg !=null)
            {
                ObjAuditOrg.AuditAction = Action;
                ObjAuditOrg.AuditBy = CreatedUserID;
                ObjAuditOrg.AuditTimeStamp = DateTime.Now;
                ObjAuditOrg.OrgCode = ObjUsrOrg.OrgCode;
                ObjAuditOrg.UserOrgID = ObjUsrOrg.UserOrgID; 
                ObjAuditOrg.OrgName = ObjUsrOrg.OrgName;
                ObjAuditOrg.Status = ObjUsrOrg.Status;
                ObjAuditOrg.UserID = ObjUsrOrg.UserID;
                db.A_UserOrgs.InsertOnSubmit(ObjAuditOrg);
                db.SubmitChanges();
            }
        }
        //public void AuditUserProject(string ProjectCode, string AssignedUserID, string CreatedUserID, string Action)
        //{
        //    UserProject ObjUsrOrg = db.UserProjects.SingleOrDefault(x => x.ProjectCode == ProjectCode && x.UserID == AssignedUserID);
        //    A_UserOrg ObjAuditOrg = new A_UserOrg();
        //    if (ObjUsrOrg != null)
        //    {
        //        ObjAuditOrg.AuditAction = Action;
    ...
    public partial class Project
    {
        ...
        public string getProjectName(string ProjectCode, string orgID)
        {
            string orgCode = string.Empty;
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CS"].ToString()))
                {
                    using (SqlCommand cmd = new SqlCommand("FIRMS_VerifyProjectsUsingProjectCode", con))
                    {
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                orgCode = dt.Rows[0]["depm_desc"].ToString();
                            }
                        }
                        con.Close();
                    }
        ...
        public string getOrganizationName(string orgCode)
        {
            ...
            {
                string value = string.Empty;
                var masg = db.FIRMS_getOrgsName(orgCode);
                foreach (var m in masg)
                {
                    orgName = m.org_name;
                }
        ...
        public string getBadgeID(string UserName)
        {
            ...
            try
            {
                ...
                var masg = from Usr in db.Users
                           join
                           Sec in db.SS_UserSecurityGroups on Usr.UserID equals Sec.UserID
                           where  Usr.Status == "ACT" && (Usr.UserID == UserName || Usr.FirstName == UserName)
                           select new { Usr.UserID, Usr.emp_code };

                foreach (var m in masg)
                {
                    UserID = m.emp_code.ToString();
                }
                return UserID;
            }
            catch (Exception ex)
        ...
        public string ValidateAuthorizedByUserID(int ID)
        {
            string Emp_Name = string.Empty;
            //int Emp_Id = string.Empty;
            string Emp_Designation = string.Empty;
            try
            {
                string value = string.Empty;
                var masg = db.FIRMS_ValidateEmployeeByID(ID).ToList();
                var count = masg.Count();
                foreach (var m in masg)
                {
                    Emp_Name = m.emp_name;
                    Emp_Designation = m.dgt_desig_name;
                    //Emp_Id = m.emp_code;
                }
                if (count > 0)
                { 
                return ID + ";" + Emp_Name + ";" + Emp_Designation;
                }
                else
                {
                    return "Exception: ";
        ...
        public string ValidateFromDomainTableValue(string DomainName, string Value)
        {
            ...
            try
            {
                string value = string.Empty;
                var masg = from Sec in db.SS_ALNDomains
                           where Sec.DomainName == DomainName && Sec.Value == Value && Sec.IsActive == true
                           select new { Sec.Value};
        ...
        public string ValidateFromIncotermsTableValue(string DomainName, String Value)
        {
            string IncoDesc = string.Empty;
            try
            {
                string value = string.Empty;
                var masg = from Sec in db.SS_ALNDomains
                ...
                foreach (var m in masg)
                {
                    IncoDesc = m.Value + ";" +m.Description.ToString();
                }
                return IncoDesc;
            }
            catch (Exception ex)
        ...
        public string ValidateFromDomainTableDescription1(string DomainName, string Value)
        {
            string ValueID = string.Empty;
            try
            {
                string value = string.Empty;
                var masg = from Sec in db.SS_ALNDomains
                           where Sec.DomainName == DomainName && Sec.Description == Value
                           select new { Sec.Value, Sec.Description };
        ...
        public string GetIncoterms(string Value)
        {
            string IncotermID = string.Empty;
            try
            {
                string value = string.Empty;
                var masg = from Sec in db.SS_ALNDomains
                           where Sec.DomainName == "INCOTERM" && Sec.Value == Value
                           select new { Sec.Value, Sec.Description };

                foreach (var m in masg)
                {
                    IncotermID = m.Description.ToString();
                }
                return IncotermID;
            }
            catch (Exception ex)
        ...
        public string VerifyContractID(int ContractID)
        {
            ...
            try
            {
                ...
                var masg = from Sec in db.CONTRACTs
                           where Sec.CONTRACTNUM == ContractID && Sec.STATUS=="ACT"
        ...
        public string ReturnDesignationName(short design_Code)
        {
            string designationName = "";
            if (design_Code != null)
            {
                var getdesignationvalue = db.FIRMS_GetAllDesignation().SingleOrDefault(x => x.dgt_desig_code == design_Code);
               if (getdesignationvalue != null)
               {
                   designationName = getdesignationvalue.dgt_desig_name;
               }
            }
            return designationName;
        }

        public void SaveAuditUserProject(string ProjectCode, string AssignedUserID, string CreatedBy, string Action) {
            UserProject UsrProject = db.UserProjects.SingleOrDefault(x => x.ProjectCode == ProjectCode && x.UserID == AssignedUserID);
            if (UsrProject != null)
            {
                A_UserProject ObjAuditUsrProj = new A_UserProject();
                if(ObjAuditUsrProj != null)
                {
                    ObjAuditUsrProj.AuditAction = Action;
                    ObjAuditUsrProj.AuditBy = CreatedBy;
                    ObjAuditUsrProj.AuditTimeStamp = DateTime.Now;
                    ObjAuditUsrProj.OrgCode = UsrProject.OrgCode;
                    ObjAuditUsrProj.ProjectCode = UsrProject.ProjectCode;
                    ObjAuditUsrProj.ProjectName = UsrProject.ProjectName;
                    ObjAuditUsrProj.Status = true;
                    ObjAuditUsrProj.UserID = AssignedUserID;
                    db.A_UserProjects.InsertOnSubmit(ObjAuditUsrProj);
                    db.SubmitChanges();
                }
            }
        }
    ...
    public partial class A_POSignatureTemplate
    {
        ...
        public string SaveRecordInAuditSignature(string userID, string Action, int POSignID)
        {
            ...
            try
            {
                POSignatureTemplate ObjTeam = db.POSignatureTemplates.FirstOrDefault(x => x.POSignatureTemplateID == POSignID);
                ...
                if (ObjTeam != null)
                {
                    ...
                    ObjATeam.POSignatureTemplateID = ObjTeam.POSignatureTemplateID; 
                    ObjATeam.Designation = ObjTeam.Designation;
                    ObjATeam.OrderNo = ObjTeam.OrderNo;
                    ObjATeam.Authority = ObjTeam.Authority; 
    ...
    #region A_PODefination
    public partial class A_PODefination
    {
        FSPDataAccessModelDataContext db = new FSPDataAccessModelDataContext(ConfigurationManager.ConnectionStrings["CS"].ToString());
        public string SaveRecordInAuditProjectMember(string userID, string Action, string orgCode, string DefinationContent)
        {
            string masg = string.Empty;
            try
            {
                PODefination ObjTeam = db.PODefinations.FirstOrDefault(x => x.OrgCode == orgCode && x.DefinationType == DefinationContent);

                if (ObjTeam != null)
                {
                    A_PODefination ObjATeam = new A_PODefination();
                    ObjATeam.AuditAction = Action;
                    ObjATeam.AuditBy = userID;
                    ObjATeam.PODefinationID = ObjTeam.PODefinationID;
                    ObjATeam.AuditTimeStamp = DateTime.Now;
                    ObjATeam.OrgCode = orgCode;
                    ObjATeam.OrgName = ObjTeam.OrgName;
                    ObjATeam.DefinationContent = ObjTeam.DefinationContent;
                    ObjATeam.DefinationType = ObjTeam.DefinationType;
                    db.A_PODefinations.InsertOnSubmit(ObjATeam);
                    db.SubmitChanges();
                    masg = "Success";
                }
